    ...
    return (
      <>
        <div className="w-full max-w-7xl flex-1 mx-auto rounded-lg bg-white p-2 md:p-10 flex flex-col justify-start shadow-xl mt-4 md:mt-16 overflow-auto" style={{ minHeight: '400px', maxHeight: 'calc(100vh - 6rem)' }}>
          <h2 className="text-xl md:text-3xl font-bold mb-4 md:mb-6 mt-4 md:mt-6 text-center">Pacotes</h2>
            ...
            <div className="bg-white rounded-lg shadow-2xl max-w-lg w-full p-8 relative animate-fadeIn flex flex-col gap-4 max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleCreateSubmit} className="flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-2 mt-2">
                  <span className="font-semibold">Avaliações (opcional)</span>
                  {createForm.ratings.map((r, idx) => (
                    <div key={idx} className="flex gap-2 items-center">
                      <input type="number" min="0" max="5" className="border rounded px-2 py-1 w-16" value={r.rating} onChange={e => handleRatingChange(idx, 'rating', e.target.value)} placeholder="Nota" />
                      <input type="text" className="border rounded px-2 py-1 flex-1" value={r.comment} onChange={e => handleRatingChange(idx, 'comment', e.target.value)} placeholder="Comentário" />
                      <button type="button" className="text-red-500 px-2" onClick={() => handleRemoveRatingField(idx)} title="Remover">&times;</button>
                    </div>
                  ))}
                  <button type="button" className="text-blue-600 underline text-sm w-fit" onClick={handleAddRatingField}>Adicionar avaliação</button>
                </div>
