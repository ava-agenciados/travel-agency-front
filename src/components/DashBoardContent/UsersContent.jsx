    ...
    <div className="w-full max-w-7xl flex-1 mx-auto rounded-lg bg-white p-2 md:p-10 flex flex-col justify-start shadow-xl mt-4 md:mt-16 overflow-auto" style={{ minHeight: '400px', maxHeight: 'calc(100vh - 6rem)' }}>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-2 md:px-4 py-2 text-center text-xs md:text-sm font-semibold text-gray-700 hidden md:table-cell">Email</th>
                <th className="px-2 md:px-4 py-2 text-center text-xs md:text-sm font-semibold text-gray-700">Cargo</th>
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {users.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE).map((user) => (
                <tr key={user.id} className="hover:bg-blue-50">
                  <td className="px-2 md:px-4 py-2 text-center whitespace-nowrap font-medium text-xs md:text-sm hidden md:table-cell">{user.id}</td>
                  <td className="px-2 md:px-4 py-2 text-center whitespace-nowrap text-xs md:text-sm">{user.firstName} {user.lastName}</td>
                  <td className="px-2 md:px-4 py-2 text-center whitespace-nowrap text-xs md:text-sm hidden md:table-cell">{user.email}</td>
                  <td className="px-2 md:px-4 py-2 text-center whitespace-nowrap text-xs md:text-sm">{user.role}</td>
                  <td className="px-2 md:px-4 py-2 text-center whitespace-nowrap">
                    <button className="text-blue-500 hover:underline p-1 md:p-2" onClick={() => openEditModal(user)} title="Editar">
      ...
      {editUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 p-4">
          <div className="bg-white rounded-lg shadow-2xl max-w-md w-full p-4 md:p-8 relative animate-fadeIn flex flex-col gap-4">
            ...
            <h3 className="text-xl md:text-2xl font-bold mb-2">Editar Usuário</h3>
