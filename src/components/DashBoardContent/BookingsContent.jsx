            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-2 text-center text-sm font-semibold text-gray-700">Cliente</th>
                  <th className="px-4 py-2 text-center text-sm font-semibold text-gray-700">Pacote</th>
                  <th className="px-4 py-2 text-center text-sm font-semibold text-gray-700">Data da Viagem</th>
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {paginatedBookings.map((booking, idx) => {
                  ...
                  if (booking.travelDate) {
                    ...
                    travelDate = isNaN(d) ? '-' : d.toLocaleDateString('pt-BR');
                  ...
                  return (
                    <tr key={booking.id || idx} className="hover:bg-blue-50">
                      <td className="px-4 py-2 whitespace-nowrap text-center font-medium">{booking.id}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-center">{clientInfo}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-center">{packageName}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-center">{travelDate}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-center">{booking.status || '-'}</td>
                      <td className="px-4 py-2 whitespace-nowrap flex gap-2 justify-center">
                        <button onClick={() => handleView(booking)} className="p-2 rounded hover:bg-blue-100 text-blue-600" title="Visualizar">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-6">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                          </svg>
                        </button>
                        <button onClick={() => alert('Função de editar reserva ainda não implementada.')} className="p-2 rounded hover:bg-yellow-100 text-yellow-600" title="Editar">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-6">
                            <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                          </svg>
                        </button>
                        <button onClick={() => handleDelete(booking)} className="p-2 rounded hover:bg-red-100 text-red-600" title="Excluir">
      ...
      {showModal && selectedBooking && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-lg shadow-2xl max-w-2xl w-full p-8 relative animate-fadeIn flex flex-col gap-4 max-h-[90vh] overflow-y-auto">
            ...
            <h3 className="text-2xl font-bold mb-2">Detalhes da Reserva</h3>
            <div className="flex flex-col gap-2">
              ...
              <p><span className="font-semibold">Data da Viagem:</span> {selectedBooking.travelDate ? new Date(selectedBooking.travelDate).toLocaleDateString('pt-BR') : '-'}</p>
              <p><span className="font-semibold">Status:</span> {selectedBooking.status || '-'}</p>
              <p><span className="font-semibold">Desconto:</span> {selectedBooking.discountPercent ? selectedBooking.discountPercent + '%' : '-'}</p>
              ...
              {selectedBooking.lodgingInfo && (
                <div className="mt-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-base mt-1">
