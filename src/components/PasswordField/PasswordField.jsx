...
const PasswordField = ({ 
  ...
  // Estado local para controlar visibilidade da senha (mostrar/ocultar)
  const [showPassword, setShowPassword] = useState(false)

  return (
    // Fieldset com bordas e efeito de vidro (backdrop blur)
    <fieldset className="border border-white border-opacity-40 rounded-md p-2 relative" style={{
      backgroundColor: 'rgba(255, 255, 255, 0.1)',
      backdropFilter: 'blur(10px)'
    }}>
      {/* Legend que funciona como label do campo */}
      <legend className="text-white text-sm font-medium px-2 uppercase tracking-wide" style={{ fontFamily: 'Inter, sans-serif' }}>
        {label}
      </legend>
      {/* Container relativo para posicionar o botão de toggle */}
      <div className="relative">
        {/* Input de senha com tipo dinâmico (text ou password) */}
        <input
          type={showPassword ? "text" : "password"} // Alterna entre texto visível e senha oculta
          id={id}
          name={name}
          value={value}
          onChange={onChange}
        ...
        <button
          type="button"
          onClick={() => setShowPassword(!showPassword)} // Toggle do estado showPassword
        ...
        >
          {showPassword ? (
            // Ícone de olho cortado (senha visível) - indica que clicando irá ocultar
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path>
              <line x1="1" y1="1" x2="23" y2="23"></line>
            </svg>
          ) : (
            // Ícone de olho normal (senha oculta) - indica que clicando irá mostrar
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
